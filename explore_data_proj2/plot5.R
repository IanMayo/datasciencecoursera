...
data<-aggregate(NEI$Emissions[vehicleEmissions & baltimoreData] ~ NEI$year[vehicleEmissions & baltimoreData] , NEI, sum)
...
## 7. Produce the plot
p <- ggplot(data, aes(x=factor(Year),y=Emissions)) + geom_bar(stat = "identity") +
